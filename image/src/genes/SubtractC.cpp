...
void ci::SubtractCGene::initializeParametersFromConfig(
...
void ci::SubtractCGene::initializeParametersFromTemplateGene(const cc::Gene* other) {
    const ci::SubtractCGene* gene = dynamic_cast<const ci::SubtractCGene*>(other);
    if (gene == nullptr) {
        throw std::runtime_error(
            "Attempting to initialize SubtractCGene with a gene of type '"
            + other->getGeneName()
            + "'.");
    }
...
void ci::SubtractCGene::mutateParameters() {
    value_ = randomNumberGenerator_->getRandomInt(minValue_, maxValue_);
}

void ci::SubtractCGene::evaluate(std::vector<std::shared_ptr<cc::DataChunk>>& buffers) {
...
std::string ci::SubtractCGene::generateCode(cc::CodeGenerationContext_t& context) const {
...
std::unordered_map<std::string, std::string> ci::SubtractCGene::serializeInternal() const {
