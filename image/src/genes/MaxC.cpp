...
void ci::MaxCGene::initializeParametersFromConfig(
...
void ci::MaxCGene::initializeParametersFromTemplateGene(const cc::Gene* other) {
    const ci::MaxCGene* gene = dynamic_cast<const ci::MaxCGene*>(other);
    if (gene == nullptr) {
        throw std::runtime_error(
            "Attempting to initialize MaxCGene with a gene of type '"
            + other->getGeneName()
            + "'.");
    }
...
void ci::MaxCGene::mutateParameters() {
    value_ = randomNumberGenerator_->getRandomInt(minValue_, maxValue_);
}

void ci::MaxCGene::evaluate(std::vector<std::shared_ptr<cc::DataChunk>>& buffers) {
...
std::string ci::MaxCGene::generateCode(cc::CodeGenerationContext_t& context) const {
...
std::unordered_map<std::string, std::string> ci::MaxCGene::serializeInternal() const {
