...
void ci::AddCGene::initializeParametersFromConfig(
...
void ci::AddCGene::initializeParametersFromTemplateGene(const cc::Gene* other) {
    const ci::AddCGene* gene = dynamic_cast<const ci::AddCGene*>(other);
    if (gene == nullptr) {
        throw std::runtime_error(
            "Attempting to initialize AddCGene with a gene of type '"
            + other->getGeneName()
            + "'.");
    }
...
void ci::AddCGene::mutateParameters() {
    value_ = randomNumberGenerator_->getRandomInt(minValue_, maxValue_);
}

void ci::AddCGene::evaluate(std::vector<std::shared_ptr<cc::DataChunk>>& buffers) {
...
std::string ci::AddCGene::generateCode(cc::CodeGenerationContext_t& context) const {
...
std::unordered_map<std::string, std::string> ci::AddCGene::serializeInternal() const {
