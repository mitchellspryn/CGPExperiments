...
void ci::MaxValueGene::initializeParametersFromTemplateGene(const cc::Gene* other) {
    // Nothing to initialize!
}

void ci::MaxValueGene::mutateParameters() {
    // Nothing to mutate!
}

void ci::MaxValueGene::evaluate(std::vector<std::shared_ptr<cc::DataChunk>>& buffers) {
    ...
    for (int i = 0; i < num; i++) {
        ...
        output.setTo(cv::Scalar(maxVal));
    }
}

std::string ci::MaxValueGene::generateCode(cc::CodeGenerationContext_t& context) const {
...
std::unordered_map<std::string, std::string> ci::MaxValueGene::serializeInternal() const {
